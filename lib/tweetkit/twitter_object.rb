...
module Tweetkit
  class TwitterObject
    ...
    def self.construct_from(values, opts = {})
      values = Tweetkit::Util.symbolize_names(values)

      # work around protected #initialize_from for now
      new(values[:id]).send(:initialize_from, values, opts)
    end

    # Determines the equality of two Twitter objects. Twitter objects are
    # considered to be equal if they have the same set of values and each one
    # of those values is the same.
    def ==(other)
      other.is_a?(TwitterObject) &&
        @values == other.instance_variable_get(:@values)
    end

    # Hash equality. As with `#==`, we consider two equivalent Twitter objects
    # equal.
    def eql?(other)
    ...
    # As with equality in `#==` and `#eql?`, we hash two Twitter objects to the
    ...
    #   which applies only to new TwitterObjects being initiated under this
    #   TwitterObject. Defaults to true.
    def update_attributes(values, opts = {}, dirty: true)
      values.each do |k, v|
        add_accessors([k], values) unless metaclass.method_defined?(k.to_sym)
        @values[k] = Util.convert_to_twitter_object(v, opts)
    ...
    # Sets all keys within the TwitterObject as unsaved so that they will be
    # included with an update when #serialize_params is called. This method is
    # also recursive, so any TwitterObjects contained as values or which are
    ...
    # serialized.
    def marshal_dump
      # The TwitterClient instance in @opts is not serializable and is not
      # really a property of the TwitterObject, so we exclude it when
      # dumping
      opts = @opts.clone
      opts.delete(:client)
      [@values, opts]
    ...
    def serialize_params(options = {})
      ...
      @values.each do |k, v|
        ...
        #   3. Its value is a TwitterObject. A TwitterObject may contain modified
        #      values within in that its parent TwitterObject doesn't know about.
        #
        unsaved = @unsaved_values.include?(k)
        next unless options[:force] || unsaved || v.is_a?(TwitterObject)
    ...
    # `TwitterObject`, and metadata can have a wide range of different keys
        ...
        keys.each do |k|
          ...
          [k, :"#{k}=", :"#{k}?"].each do |method_name|
            ...
            begin
              ...
              #
              #     https://github.com/twitter/twitter-ruby/issues/749
              #
    ...
    protected def method_missing(name, *args)
      ...
      begin
        ...
        raise NoMethodError,
              ...
              "with the result returned by Twitter's API, probably as a " \
    ...
    # * +:opts:+ Options for TwitterObject like an API key.
    ...
    protected def serialize_params_value(value, original, unsaved, force,
      ...
      # making sure any time one is set, we convert it to a TwitterObject. This
      ...
      # existing array being held by a TwitterObject. This could happen for
      # example by appending a new hash onto `additional_owners` for an
      # account.
      elsif value.is_a?(Hash)
        Util.convert_to_twitter_object(value, @opts).serialize_params

      elsif value.is_a?(TwitterObject)
    ...
    private def empty_values(obj)
      values = case obj
               when Hash         then obj
               when TwitterObject then obj.instance_variable_get(:@values)
               else
                 raise ArgumentError,
